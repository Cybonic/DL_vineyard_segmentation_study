...
Repository with other loss functions : https://github.com/JunMa11/SegLoss/blob/master/losses_pytorch/dice_loss.py
...
def TEST_DICE_LOSS(batch_size,n_class,dim):
    print(dim)
    print(n_class)
    input = torch.randn(batch_size,n_class,dim[0],dim[1], requires_grad=True,dtype=torch.float32)   
    target = torch.randint(0,1,(batch_size,n_class,dim[0],dim[1]),requires_grad=True,dtype=torch.float32)
    criterion = GDiceLoss()
